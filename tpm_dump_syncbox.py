...
PHASE_0_MAP = [[0, "AAVS1_LOOP"], [1, "DIRECT"]]
...
if __name__ == "__main__":
	...
	try:
		...
		plt.ioff()
		fig, p4_ax = plt.subplots(nrows=2, ncols=1, figsize=(12, 7), facecolor='w')
		p4_ax = p4_ax.reshape(1,2)[0]
		plt.tight_layout(rect=[0, 0.03, 1, 0.95])
		fig.show()
		...
		while True:
			...
			## Creating Directory to store the videos
			if not os.path.exists(OUT_PATH + "SYNC_IMG"):
				os.makedirs(OUT_PATH + "SYNC_IMG")
				os.makedirs(OUT_PATH + "SYNC_IMG/PLOT-A")
			...
			for i in TPMs:
				...
				fig.canvas.draw()
				time.sleep(1)
				plt.savefig(OUT_PATH + "SYNC_IMG/PLOT-A/LB_SYNC_PHASE-0_A_" + ora + ".png")
				time.sleep(1)
