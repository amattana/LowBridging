        ...
        if len(lista):
            for n, l in enumerate(lista):
                if not n == len(lista) - 1:
                    ...
                    print f_next, dt_to_timestamp(datetime.datetime.strptime(f_next, "%Y-%m-%d_%H%M%S"))
                    if t_start >= dt_to_timestamp(datetime.datetime.strptime(f_next, "%Y-%m-%d_%H%M%S")):
                        pass
                    else:
                        f_date = l.split("/")[-1][:17]
                        print f_date, dt_to_timestamp(datetime.datetime.strptime(f_date, "%Y-%m-%d_%H%M%S"))
