...
if __name__ == "__main__":
    ...
    # exit()
        ...
        if options.power:
            ...
            pfile = open(datapath + "/../../../POWER/CH-" + str(int(options.channel)) + "/POWER_CH-" + str(
                int(options.channel)) + "_" + RX_DIR[:-1] + "_" + POL_DIR[:-1] + ".csv", "w")
            ...
            if options.framerate == 0:
                ...
                if ((orario - ora_inizio).seconds / 60.) > options.framerate:
                    ...
                    if len(spgramma) > 5:
                        if options.water:
                            ax1.cla()
                            ax1.imshow(spgramma, interpolation='none', aspect='auto',
                                       extent=[asse_x[xmin], asse_x[xmax], options.framerate, 0], cmap='jet', clim=wclim)
                            ax1.set_title(" Spectrogram of " + str(len(spgramma)) + " spectra")
                            ax1.set_ylabel("Time (minutes)")
                            ax1.set_xlabel('MHz')
        ...
        ax1.cla()
        if options.water:
            ax1.imshow(dayspgramma, interpolation='none', aspect='auto', extent=[asse_x[xmin], asse_x[xmax], 1, 0],
                       cmap='jet', clim=wclim)
        ...
        plt.savefig(BASE_DIR + "SPECTROGRAM/" + SPG_DIR + "/" + RX_DIR + POL_DIR + SPG_DIR[13:] + "_" + fname.split("/")[-1][
                                                                                                        :-11] + ".png")
        if options.power:
            pfile.close()
